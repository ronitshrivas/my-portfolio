...
/// Search data source backed by Innovator's own .NET search service
/// (http://36.253.137.34:8015). Exposes the same method names the section
/// already calls, so the UI/state flow is unchanged — only the backing API
/// switched from the bundled stub to Innovator's real endpoints.
///
/// Innovator's search is people-focused, so posts/hashtags/history come back
/// empty (the UI already handles empty sections gracefully).
class _InnovatorSearchApi {
  ...
  SearchUserHit _hitFromJson(Map<String, dynamic> json) {
    var avatar = (json['profile']?['avatar'] ?? json['avatar'])?.toString();
    ...
    return SearchUserHit(
      ...
      bio: json['profile']?['bio']?.toString() ?? json['bio']?.toString(),
      role: json['role']?.toString(),
    );
  }

  List<SearchUserHit> _parseUsers(String body) {
  ...
  Future<List<SearchUserHit>> suggestedUsers() async {
    ...
    return _parseUsers(res.body).take(10).toList();
  }

  Future<List<SearchUserHit>> searchUsers(String q) async {
    final uri = Uri.parse(
      '${ApiConstants.searchApi}/search/users',
    ).replace(queryParameters: {'q': q.trim()});
    final res = await http.get(uri, headers: _headers);
    if (res.statusCode != 200) return const [];
    return _parseUsers(res.body);
  }
  ...
  Future<CombinedSearchResult> search({
    ...
    return CombinedSearchResult(users: users);
  }

  // Innovator's search API doesn't expose posts, hashtags, or history yet.
  Future<List<SearchPostHit>> searchPosts(String q) async => const [];
  //Future<List<SearchHashtagHit>> searchHashtags(String q) async => const [];
  Future<List<SearchHistoryItem>> history() async => const [];
...
class _SearchSectionState extends State<SearchSection>
  ...
  final _searchApi = _InnovatorSearchApi();
    ...
    try {
      ...
      setState(() {
        _results = CombinedSearchResult(
          ...
          hashtags: hashtags,
          totalUsers:
              combined.totalUsers > 0 ? combined.totalUsers : users.length,
          totalPosts:
              combined.totalPosts > 0 ? combined.totalPosts : posts.length,
        );
        _loading = false;
      });
...
class _UserTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final subtitle =
        subtitleOverride?.trim().isNotEmpty == true
...
class _ErrorBlock extends StatelessWidget {
  const _ErrorBlock({super.key, required this.message, required this.onRetry});
