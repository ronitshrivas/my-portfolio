...
class _FacebookVideoWidgetState extends State<FacebookVideoWidget>
  ...
  Future<void> _initPlayer() async {
    ...
    try {
      ...
      // ↓ Read real size from initialized controller
      final size = controller.value.size;
      double rawRatio = (size.height > 0) ? size.width / size.height : 16 / 9;

      // ── FIX: Rotation metadata strip ─────────────────────────────────────
      // When videos are recorded in portrait (9:16) on mobile and uploaded,
      // some server/CDN pipelines strip the EXIF rotation tag. The video
      // then reports landscape dimensions (e.g. 1920×1080) even though it
      // should be portrait (1080×1920). This causes "stretched" playback.
      //
      // Detection: if the reported ratio is > 1.3 (landscape) but the
      // video was likely shot portrait, we check if flipping gives a
      // ratio that looks like standard portrait (0.4–0.8). If so, flip.
      //
      // We use 0.75 (4:3) as the upper threshold — any wider-than-4:3
      // landscape that maps to a portrait-ish ratio when inverted gets
      // corrected. Genuine wide-screen videos (movies etc.) are > 1.5
      // and their inverse is < 0.67 — they pass through unchanged only
      // when the inverse doesn't look portrait. We therefore only flip
      // when the raw ratio is between 1.3 – 2.0 (phone-portrait-suspect).
      final double realRatio;
      if (rawRatio > 1.3 && rawRatio < 2.1) {
        final flipped = 1.0 / rawRatio; // ~0.56 for 9:16
        // If the flipped ratio looks portrait (taller than wide)
        if (flipped < 0.8) {
          realRatio = flipped; // correct to portrait
        } else {
          realRatio = rawRatio;
        }
      } else {
            ...
            return SizedBox(
              ...
              child: ColoredBox(
                ...
                child: Stack(
                  ...
                  children: [
                    // ── Video or placeholder ─────────────────────────────
                    // FIX: Use the corrected _aspectRatio (rotation-aware),
                    // not the raw controller ratio which may be wrong when
                    // server strips EXIF rotation from portrait videos.
                    if (_initialized && _controller != null)
                      SizedBox.expand(
                        child: FittedBox(
                          // cover = fill the card, crop sides if needed
                          // (same behaviour as Instagram / TikTok feed)
                          fit:
                              _aspectRatio < 1.0
                                  ? BoxFit
                                      .fitWidth // portrait video → fill width
                                  : BoxFit
                                      .contain, // landscape → contain (no crop)
                          child: SizedBox(
                            // Give VideoPlayer its correct pixel dimensions so
                            // FittedBox can calculate the right scale factor.
                            width: _aspectRatio >= 1.0 ? width : width,
                            height:
                                _aspectRatio >= 1.0
                                    ? width / _aspectRatio
                                    : width / _aspectRatio,
