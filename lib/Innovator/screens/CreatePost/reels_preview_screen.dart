...
//   String? _mergedVideoPath; // path after ffmpeg mix; null = use raw video
...
//       if (selectedMusic != null && selectedMusic.audioUrl.isNotEmpty) {
//         _startMusicPlayback(selectedMusic);
//         // Start merging in background so it's ready when user taps Share
//         _mergeAudioIntoVideo(selectedMusic.audioUrl);
//       }
//     });
//   }
...
//     await ctrl.initialize();
//     if (ctrl.value.size.isEmpty) {
//       debugPrint('Video size empty after initialize!');
//       return;
//     }
//     ctrl.setLooping(true);
//     final hasMusic = ref.read(reelsProvider).selectedMusic != null;
...
//   // ── FIX: Merge selected music audio into video using ffmpeg ───────────────
//   //
//   // This runs as soon as the preview screen opens so by the time the user
//   // taps "Share" the merged file is already ready — no extra wait.
//   //
//   // FFmpeg command breakdown:
//   //   -i videoPath      → input: the recorded silent video
//   //   -i musicUrl       → input: Deezer 30s preview MP3 (URL, direct input)
//   //   -c:v copy         → copy video stream unchanged (no re-encode, fast!)
//   //   -c:a aac          → encode audio as AAC (standard mobile format)
//   //   -map 0:v:0        → use video from first input
//   //   -map 1:a:0        → use audio from second input (the music)
//   //   -shortest         → stop when the shorter stream ends
//   //   -y                → overwrite output if exists
//   // ─────────────────────────────────────────────────────────────────────────
//   Future<void> _mergeAudioIntoVideo(String musicUrl) async {
//     if (_isMerging) return;
//     if (mounted) setState(() => _isMerging = true);

//     try {
//       final dir = await getTemporaryDirectory();
//       final outputPath =
//           '${dir.path}/reel_merged_${DateTime.now().millisecondsSinceEpoch}.mp4';

//       // Download music to temp file first (more reliable than passing URL to
//       // ffmpeg on mobile networks)
//       final audioPath = await _downloadAudio(musicUrl);
//       if (audioPath == null) {
//         debugPrint(
//           '[FFmpeg] Audio download failed — uploading video without music',
//         );
...
//       final cmd =
//           '-i "${widget.videoPath}" -i "$audioPath" '
//           '-c:v copy -c:a aac -b:a 128k '
//           '-map 0:v:0 -map 1:a:0 -shortest -y "$outputPath"';

//       debugPrint('[FFmpeg] Running: $cmd');
//       final session = await FFmpegKit.execute(cmd);
//       final rc = await session.getReturnCode();

//       if (ReturnCode.isSuccess(rc)) {
//         debugPrint('[FFmpeg] Merge SUCCESS → $outputPath');
//         if (mounted) {
//           setState(() {
...
//   /// Download audio from URL to a temp .mp3 file. Returns path or null.
//   Future<String?> _downloadAudio(String url) async {
//     try {
//       final dir = await getTemporaryDirectory();
//       final audioPath =
//           '${dir.path}/reel_audio_${DateTime.now().millisecondsSinceEpoch}.mp3';
...
//     // ── If still merging, wait for it (max 30s) ──────────────────────────
//     if (_isMerging) {
//       if (mounted) {
//         ScaffoldMessenger.of(context).showSnackBar(
//           const SnackBar(
...
//                 ),
//                 SizedBox(width: 12),
//                 Text('Preparing music, please wait…'),
//               ],
//             ),
...
//     final String captionText = _captionCtrl.text.trim();
//     // ── Use merged video if available, else raw video ─────────────────────
//     final String uploadPath = _mergedVideoPath ?? widget.videoPath;
//     final String? accessToken = AppData().accessToken;
//     final ReelsMusicTrack? selectedMusic =
//         ref.read(reelsProvider).selectedMusic;
//     final container = ProviderScope.containerOf(context);
...
//         request.headers['Authorization'] = 'Bearer $accessToken';
//       }
//       if (captionText.isNotEmpty) request.fields['caption'] = captionText;

//       // ── Send music metadata as fields (for feed display / analytics) ───
//       // The actual audio is now baked into the video file itself
//       if (selectedMusic != null && _mergedVideoPath != null) {
//         request.fields['music_title'] = selectedMusic.title;
//         request.fields['music_artist'] = selectedMusic.artist;
//         request.fields['has_audio_merged'] = 'true';
//       } else if (selectedMusic != null) {
//         // Fallback: backend merge if ffmpeg failed
//         request.fields['music_url'] = selectedMusic.audioUrl;
//         request.fields['music_title'] = selectedMusic.title;
//         request.fields['music_artist'] = selectedMusic.artist;
//       }

//       final mimeType = lookupMimeType(uploadPath) ?? 'video/mp4';
//       request.files.add(
//         await http.MultipartFile.fromPath(
//           'video',
//           uploadPath,
...
//       // Clean up merged file after upload
//       if (_mergedVideoPath != null) {
//         try {
//           File(_mergedVideoPath!).deleteSync();
//         } catch (_) {}
//       }
...
//           // ── Music merge progress indicator ───────────────────────────
//           if (_isMerging)
//             Positioned(
...
//     // ── FIX: Always treat reel videos as portrait ─────────────────────────
//     // Camera records in portrait but the underlying MP4 may report landscape
//     // (rotation metadata). We always force portrait dimensions here.
//     final double videoW =
//         videoSize.width < videoSize.height ? videoSize.width : videoSize.height;
//     final double videoH =
//         videoSize.width > videoSize.height ? videoSize.width : videoSize.height;
...
//                 const SizedBox(width: 10),
//                 GestureDetector(
//                   onTap: (_isUploading || _isMerging) ? null : _uploadReel,
//                   child: Container(
//                     padding: const EdgeInsets.symmetric(
//                       horizontal: 20,
//                       vertical: 10,
//                     ),
//                     decoration: BoxDecoration(
//                       color: _isMerging ? _orange.withOpacity(0.6) : _orange,
//                       borderRadius: BorderRadius.circular(24),
//                     ),
//                     child:
//                         _isUploading
...
//             final music = ref.read(reelsProvider).selectedMusic;
//             if (music != null) {
//               // Reset merged video and start fresh merge with new track
//               setState(() {
//                 _mergedVideoPath = null;
//                 _isMerging = false;
//               });
//               await _startMusicPlayback(music);
//               _videoCtrl?.setVolume(0.0);
//               _mergeAudioIntoVideo(music.audioUrl);
//             }
//           }
//         },
//       ),
