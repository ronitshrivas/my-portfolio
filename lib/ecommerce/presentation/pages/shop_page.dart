...
import 'package:innovator/ecommerce/providers/ecommerce_providers.dart'
    show mapToShopProduct;
...
class _ShopSectionState extends State<ShopSection>
  ...
  // Live products from the ecommerce backend (replaces the old mock catalog).
  final EcommerceApi _shopApi = EcommerceApi();
  List<ShopProduct> _products = [];

  // Admin-managed banners; drive the featured carousel when present.
  List<AppBanner> _banners = const [];

  Future<void> _loadProducts() async {
    try {
      final dtos = await _shopApi.products();
      // ignore: avoid_print
      print('[Shop] loaded ${dtos.length} products');
      if (!mounted) return;
      setState(() => _products = dtos.map(mapToShopProduct).toList());
    } catch (e) {
      // ignore: avoid_print
      print('[Shop] product load error: $e');
    }
  }

  Future<void> _loadBanners() async {
    try {
      final banners = await _shopApi.banners();
      if (!mounted) return;
  ...
  void _openBanner(AppBanner banner) {
    ...
    if (match != null) _openProductDetail(match);
  }
  ...
  void _startAutoScroll() {
    ...
    _autoScroll = Timer.periodic(const Duration(seconds: 3), (_) {
      ...
      final total = _banners.isNotEmpty ? _banners.length : _featured.length;
      if (total == 0) return;
  ...
  Widget _buildFeatured() {
    ...
    final count = useBanners ? _banners.length : _featured.length;
    return Column(
      children: [
        SizedBox(
          height: 176,
...
/// Admin-managed promo banner: full-bleed image that opens the linked product.
